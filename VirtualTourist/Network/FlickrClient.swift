...
class FlickrClient {
    static let key = "place_key_here"
    static let secret = "place_secret_here"
    
    // 42.2808° N, 83.7430° W
    // https://www.flickr.com/services/rest/?method=flickr.photos.getrecent&api_key=my_key&lat=42.2808&lon=83.7430&radius=20&per_page=10&page=1&format=json&nojsoncallback=1&extras=url_m
    enum Endpoints {
        static let baseUri = "https://www.flickr.com/services/rest/?method=flickr.photos.getrecent"
        ...
        var urlString: String {
            ...
            case .searchURLString(let latitude, let longitude, let perPage, let pageNum):
                return Endpoints.baseUri + "&api_key=\(FlickrClient.key)" +
                    "&lat=\(latitude)" +
                    "&lon=\(longitude)" +
    ...
    class func getPhotos(latitude: Double, longitude: Double, totalPageAmount: Int = 0, completion: @escaping ([FlickrPhoto], Int, Error?) -> Void) -> Void {
        let url = getFlickrURL(latitude: latitude, longitude: longitude, totalPageAmount: totalPageAmount)
        _ = taskForGETRequest(url: url, responseType: FlickrResponse.self) { response, error in
    ...
    class func taskForGETRequest<ResponseType: Decodable>(url: URL, responseType:
        ResponseType.Type, completion: @escaping (ResponseType?, Error?) -> Void) -> URLSessionDataTask {
